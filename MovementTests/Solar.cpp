...
//GLfloat near 
//=
//1.0f; // The near (Z Axis) point of our viewing frustrum (default 1.0f)
//GLfloat far 
//=
//1500.0f; // The far  (Z Axis) point of our viewing frustrum (default 1500.0f)
GLfloat neari = 1.0f;
GLfloat fari = 1500.0f;
...
void initGL()
{
	...
	GLfloat fH = tan(float(fieldOfView / 360.0f * 3.14159f)) * neari;
	GLfloat fW = fH * aspectRatio;
	glFrustum(-fW, fW, -fH, fH, neari, fari);
...
void drawScene()
{
	...
	glutSolidSphere(15.0, sphereLatitudalSegments, sphereLongitudalSegments);
	glEnable(GL_LIGHTING);
...
int main(int argc, char** argv)
{
	...
	window = glfwCreateWindow(GLsizei(windowWidth), GLsizei(windowHeight), "Solar", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		exit(-1);
